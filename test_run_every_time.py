...
def main(f):
    ...
    for i in range(sampling_num):
        env.init_state()
        worker.env = env
        _, _, results = worker.run_episode_every_time(training=training, sample=sample, max_waiting=max_waiting, cooperation=cooperation, render=render, disable_mask=disable_mask)
        # print(results)
...
if __name__ == "__main__":
    ...
    if not os.path.exists(target_file):
        ...
        env = TaskEnv(per_species_range=(5, 10), species_range=(1, 5), tasks_range=(20, 20), traits_dim=5, max_task_size=5, duration_scale=1, seed=i, single_ability=True, heterogeneous_speed=True)
